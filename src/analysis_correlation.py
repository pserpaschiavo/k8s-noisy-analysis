...
class CorrelationAnalysisStage(PipelineStage):
    ...
    def _execute_implementation(self, data: Optional[pd.DataFrame], all_results: Dict[str, Any], round_id: Optional[str] = None) -> Dict[str, Any]:
        """Execute correlation and covariance analyses and save CSV artifacts under a 'csv' subfolder."""
        ...
        cov_results_dfs: list[pd.DataFrame] = []
        ...
        for metric in metrics:
            for phase in phases:
                ...
                if not corr_matrix.empty:
                    corr_path = os.path.join(csv_dir, f"correlation_matrix_{metric}_{phase}.csv")
                    ...
                    corr_tidy = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)).stack().reset_index()
                ...
                if not cov_matrix.empty:
                    cov_path = os.path.join(csv_dir, f"covariance_matrix_{metric}_{phase}.csv")
                    ...
                    cov_matrix.index.name = 'tenant1'
                    cov_matrix.columns.name = 'tenant2'
                    cov_tidy = cov_matrix.where(np.triu(np.ones(cov_matrix.shape), k=1).astype(bool)).stack().reset_index()
                    cov_tidy.columns = ['tenant1', 'tenant2', 'covariance']
                    cov_tidy['metric'] = metric
                    cov_tidy['phase'] = phase
                    cov_results_dfs.append(cov_tidy)

        # Consolidate results
        if all_results_dfs:
            consolidated_df = pd.concat(all_results_dfs, ignore_index=True)
            if 'round_id' not in consolidated_df.columns:
                consolidated_df['round_id'] = round_id
            summary_path = os.path.join(csv_dir, f"correlation_summary_{round_id}.csv")
            ...
            if cov_results_dfs:
                cov_summary = pd.concat(cov_results_dfs, ignore_index=True)
                if 'round_id' not in cov_summary.columns:
                    cov_summary['round_id'] = round_id
                cov_summary_path = os.path.join(csv_dir, f"covariance_summary_{round_id}.csv")
                cov_summary.to_csv(cov_summary_path, index=False)
                self.logger.info(f"Consolidated covariance results for round {round_id} saved to {cov_summary_path}")

            return {"correlation_results": consolidated_df, "artifacts": generated_artifacts}
